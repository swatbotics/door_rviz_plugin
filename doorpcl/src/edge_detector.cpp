                        ...
                        u0( -1), v0(-1)
{
    ...
    SimpleConfig config( configFile );
...
void EdgeDetector::doorMouseClick ( int u, int v)
{
    ...
    if (drawPoints.size() == 4 && current_grasp_index < 0 ){
        ...
        getDoorInfo(height, width, doorPos, doorRot);
    }
}
...
void EdgeDetector::inputPointCloud( const PointCloud::ConstPtr & cloud,
    ...
    //waitAndDisplay();
    //cout << "making display image" << endl;
    makeDisplayImage();
...
void EdgeDetector::waitAndDisplay ()
{
    ...
    while (this->waiting)
    {
        ...
        //TODO : display the image from here
    }
}
...
void EdgeDetector::getHandlePoints( )
{
    ...
    //cout << "handle0[0]: " << handle0[0] << "\thandle0[1] 
    ...
    //TODO : we need to send the points here,
    // so that the handle can be drawn
}
...
void EdgeDetector::makeDisplayImage(){
    ...
    //cout << "is currentIntensityImage NULL? " << (currentIntensityImage.data == NULL) << endl;
